...
var memcacheKeyToServerId int
var memcacheListToServerId int
var pluginObj *pluginTest.BifrostManager
var memcacheServer *string
...
func main(){
	...
	table_name = flag.String("table", "binlog_field_memcache_plugin_test", "-table")
	...
	memcacheServer = flag.String("memcacheServer", "127.0.0.1:11211", "-memcacheServer")
	...
	var dbName = "memcacheTest_"+strconv.FormatInt(time.Now().Unix(),10)

	var(
		httpServerKey string = "httpToserverTest_111111"
		memcacheServerKey string = "memcacheToserverTest_111111"
		httpPluginPamram map[string]interface{} = make(map[string]interface{},0)
		memcacheKeyPluginPamram map[string]interface{} = make(map[string]interface{},0)
	)

	memcacheKeyPluginPamram["KeyConfig"] = "{$SchemaName}-{$TableName}-{$id}"
	memcacheKeyPluginPamram["DataType"] = "json"
	memcacheKeyPluginPamram["Type"] = "set"
	...
	pluginObj.AddToServer(memcacheServerKey,"memcache",*memcacheServer,memcacheServerKey)
	...
	b,memcacheKeyToServerId= pluginObj.AddTableToServer(dbName,*schema_name,*table_name,memcacheServerKey,"memcache",fieldList,1,memcacheKeyPluginPamram)
	if b == false{
		log.Println(dbName,*schema_name,*table_name,"add memcache toserver:",httpServerKey,false)
		runtime.Goexit()
	}
	...
	for sig := range signals {
		...
		pluginObj.DelTableToServer(dbName,*schema_name,*table_name,memcacheServerKey,memcacheKeyToServerId)
		...
		pluginObj.DelToServer(memcacheServerKey)
		...
		if resultKey.insert == true{
			log.Println("memcache key insert test success")
		}else{
			log.Println("memcache key insert test failed")
		}

		if resultKey.update == true{
			log.Println("memcache key update test success")
		}else{
			log.Println("memcache key update test failed")
		}

		if resultKey.delete == true{
			log.Println("memcache key delete test success")
		}else{
			log.Println("memcache key delete test failed")
		}

		if resultKey.query == true{
			log.Println("memcache key query test success")
		}else{
			log.Println("memcache key query test failed")
		}
...
func memcacheGet(key string) string{
	data, err := memcacheClient.Get(key)
	if err != nil{
		log.Println("memcacheGet ",key," err:",err)
		return ""
	}
...
func insert(c string) error{
	...
	content := memcacheGet(key)
	if content != ""{
		log.Println("memcache get ",key," value:",content)
