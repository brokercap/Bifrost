...
func TestEventParser_GetQueryTableName(t *testing.T) {
  ...
  ADD KEY gate_auth_id (gate_auth_id) USING BTREE;`
	SchemaName,TableName,isReName = parser.GetQueryTableName(sql)

	if SchemaName == "" && TableName == "tableTestName" && isReName == false {
	...
	if SchemaName == "db" && TableName == "tableTestName" && isReName == false {
	...
	if SchemaName == "" && TableName == "tableTestName" && isReName == false {
		t.Log("TRUNCATE tableTestName success")
	}else{
